...
ggplot(df, aes(x = values)) +
  ...
  geom_vline(aes(xintercept = mu_1), color = "hotpink", linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = mu_2), color = "orchid", linetype = "dashed", size = 1) +
  labs(title = "Plot de densité de vec_4",
       x = "Valeurs",
       y = "Densité") +
  theme_minimal() +
  annotate("text", x = mu_1, y = 0.04, label = paste("mu 1 (", mu_1, ")", sep = ""), color = "hotpink") +
  annotate("text", x = mu_2, y = 0.04, label = paste("mu 2 (", mu_2, ")", sep = ""), color = "orchid")
...
```
Le graphique de densité nous permet de choisir 10 et 2 comme moyenne d'initialisation et 5 et 5 pour les sigma et 250/250 pour les proportions.
...
# for (i in seq_along(vect1)) {
#   df[1,i] <- vect1[i]
#   df[2,i] <- proba_appart(vect1[i],theta[1],theta[3] ,theta[2],theta[4],theta[5],theta[6])[1]
#   df[3,i] <- proba_appart(vect1[i],theta[1],theta[3] ,theta[2],theta[4],theta[5],theta[6])[2]
#   
# }
# df <-t(df)
...
#   theta
#   for (i in range(500)) {
#     df[i,2] <- proba_appart(df[i,1],theta[1],theta[3] ,theta[2],theta[4],theta[5],theta[6])[1]
#     df[i,3] <- proba_appart(df[i,1],theta[1],theta[3] ,theta[2],theta[4],theta[5],theta[6])[2]
#   }
# 
#   }
#   return(theta)
...
```
```{r}
# E_M <- function(vect1, n_iter) {
#   df <- as.data.frame(matrix(0, nrow = length(vect1), ncol = 3))
#   colnames(df) <- c("x", "w1", "w2")
# 
#   # Initialisation avec des paramètres arbitraires
#   for (i in seq_along(vect1)) {
#     probs <- proba_appart(vect1[i], 10, 5, 2, 5, 0.5, 0.5)
#     df[i, ] <- c(vect1[i], probs[1], probs[2])
#   }
# 
#   # Boucle EM
...
#       probs <- proba_appart(df[i, 1], theta[1], theta[3], theta[2], theta[4], theta[5], theta[6])
#       df[i, 2] <- probs[1]
#       df[i, 3] <- probs[2]
#     }
#   }
...
```{r}
E_M <- function(vect1, n_iter = 100, mu1_init = 10, mu2_init = 2, sigma1_init = 5, sigma2_init = 5, p1_init = 0.5, p2_init = 0.5) {
...
E_M <- function(vect1, n_iter = 100, mu1_init = 10, mu2_init = 2, sigma1_init = 5, sigma2_init = 5, p1_init = 0.5, p2_init = 0.5, tol = 1e-6) {
  ...
  # Initialisation du data frame pour stocker les données et les probabilités d'appartenance
  df <- data.frame(
    x = vect1,
    w1 = rep(NA, length(vect1)),
    w2 = rep(NA, length(vect1))
  ...
  # Fonction pour calculer les probabilités d'appartenance
  proba_appart <- function(x, mu1, sigma1, mu2, sigma2, p1, p2) {
    proba_grp_1 <- p1 * dnorm(x, mean = mu1, sd = sigma1)
    proba_grp_2 <- p2 * dnorm(x, mean = mu2, sd = sigma2)
    C_1 <- proba_grp_1 / (proba_grp_1 + proba_grp_2)
    C_2 <- proba_grp_2 / (proba_grp_1 + proba_grp_2)
    return(c(C_1, C_2))
  }
  ...
  for (iter in 1:n_iter) {
    ...
    mu1_new <- weighted.mean(df$x, df$w1)
    mu2_new <- weighted.mean(df$x, df$w2)
    sigma1_new <- sqrt(sum(df$w1 * (df$x - mu1_new)^2) / sum(df$w1))
    sigma2_new <- sqrt(sum(df$w2 * (df$x - mu2_new)^2) / sum(df$w2))
    p1_new <- mean(df$w1)
    p2_new <- mean(df$w2)
    ...
    # Critère de convergence : variation relative des paramètres
    delta_mu1 <- abs(mu1_new - mu1) / abs(mu1) 
    delta_mu2 <- abs(mu2_new - mu2) / abs(mu2)
    delta_sigma1 <- abs(sigma1_new - sigma1) / abs(sigma1)
    delta_sigma2 <- abs(sigma2_new - sigma2) / abs(sigma2)
    delta_p1 <- abs(p1_new - p1) / abs(p1) 

    #et variation de la log-vraisemblance
    delta_ll <- abs(ll_new - ll_old) / (abs(ll_old) + 1e-10)

    # Si tous les deltas sont inférieurs à la tolérance, on arrête la boucle
    if (delta_mu1 < tol && delta_mu2 < tol && delta_sigma1 < tol && delta_sigma2 < tol && delta_p1 < tol && delta_ll < tol) {
      cat(sprintf("Convergence atteinte à l'itération %d.\n", iter))
      break
    }
...
resultats_EM <- E_M(vect1, n_iter = 100, tol = 0.1)
print(resultats_EM)
